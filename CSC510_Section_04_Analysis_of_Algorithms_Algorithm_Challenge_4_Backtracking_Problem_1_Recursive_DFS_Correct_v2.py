...
def dfs_recursive(list_list_given: List[List[int]],
    ...
    for index_col, value in enumerate(row_current):
        ...
        if len(list_solution_possible) > index_list_solution_possible:
            ...
            # Check if the sum of the oth solution is equal to the sum of the most recently added solution
            if len(list_solution_possible) > 1:

                """
                If the most recently added solution does not have the same sum as the oth solution then change 
                sum_same to False.
                """
